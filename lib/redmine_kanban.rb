require 'redmine'

module RedmineKanban
  PLUGIN_ID = 'redmine_kanban'.freeze
end

require_relative 'redmine_kanban/permission_policy'
require_relative 'redmine_kanban/project_catalog'
require_relative 'redmine_kanban/board_context'
require_relative 'redmine_kanban/board_membership_resolver'
require_relative 'redmine_kanban/board_workflow_status_resolver'
require_relative 'redmine_kanban/tracker_metadata_builder'
require_relative 'redmine_kanban/board_entity_reader'
require_relative 'redmine_kanban/board_count_reader'
require_relative 'redmine_kanban/project_tracker_reader'
require_relative 'redmine_kanban/board_snapshot_telemetry'
require_relative 'redmine_kanban/board_data'
require_relative 'redmine_kanban/board_metadata'
require_relative 'redmine_kanban/board_issue_presenter'
require_relative 'redmine_kanban/issue_entity_presenter'
require_relative 'redmine_kanban/mutation_result_builder'
require_relative 'redmine_kanban/ancestor_issue_updates'
require_relative 'redmine_kanban/mutation_outcome'
require_relative 'redmine_kanban/board_lists_builder'
require_relative 'redmine_kanban/param_normalizer'
require_relative 'redmine_kanban/service_response'
require_relative 'redmine_kanban/bulk_payload_normalizer'
require_relative 'redmine_kanban/issue_workflow'
require_relative 'redmine_kanban/issue_parent_attributes'
require_relative 'redmine_kanban/priority_propagation'
require_relative 'redmine_kanban/issue_mover'
require_relative 'redmine_kanban/issue_updater'
require_relative 'redmine_kanban/issue_creator'
require_relative 'redmine_kanban/issue_destroyer'
require_relative 'redmine_kanban/bulk_idempotency'
