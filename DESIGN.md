...
### Font Families
- **Primary UI**: `DM Sans`, with Japanese text using `Noto Sans JP` alongside it, and fallbacks: `Helvetica Neue, Helvetica, Arial`
...
- **Japanese text**: Use `Noto Sans JP` together with `DM Sans` for Japanese UI copy so Latin and Japanese text stay visually consistent.
