...
export function BulkSubtaskEditor({ labels, trackers, disabled = false, showRowTrackerButton = true, initialTrackerId, className, onChange, onValidationChange }: Props) {
  ...
  const enterTable = () => {
    ...
    setNotice(converted.restoredCount > 0
      ? labels.bulk_subtask_restored.replace('%{count}', String(converted.restoredCount))
      : null);
    setMode('table');
  ...
  const leaveTable = () => {
    ...
    setMode('text');
    setNotice(labels.bulk_subtask_preserved);
  };
  ...
  return (
    <section
      ...
      aria-label={labels.bulk_subtask_title}
    >
      <div className="rk-bulk-subtask-header">
        <div className="rk-bulk-subtask-mode-switch" role="group" aria-label={labels.bulk_subtask_mode}>
          <button
            type="button"
          ...
          >
            ...
            {labels.bulk_subtask_table_mode}
          </button>
          <button
            type="button"
          ...
          >
            ...
            {labels.bulk_subtask_text_mode}
          </button>
        </div>
      </div>
      {mode === 'text' ? (
        <label className="rk-field">
          <span className="rk-label">{labels.bulk_subtask_default_tracker}</span>
          <select aria-label={labels.bulk_subtask_default_tracker} value={defaultTrackerId || ''} onChange={(event) => { defaultTrackerExplicitlyChangedRef.current = true; setDefaultTrackerId(Number(event.target.value)); }} disabled={disabled}>
            <option value="">{labels.select_tracker}</option>
      ...
      {mode === 'text' ? (
        <>
          ...
          <div className="rk-bulk-subtask-count">{inputs.length} {labels.bulk_subtask_count}</div>
          {preservedDrafts.length > 0 ? <div className="rk-bulk-subtask-notice">✓ {notice ?? labels.bulk_subtask_preserved}</div> : null}
          {showRowTrackerButton ? <button type="button" className="rk-btn rk-btn-sm" onClick={enterTable} disabled={disabled}>{labels.bulk_subtask_edit_rows}</button> : null}
        </>
      ) : (
        <>
          <div className="rk-bulk-subtask-table-wrap">
            <table className="rk-bulk-subtask-table"><thead><tr><th>#</th><th>{labels.issue_subject}</th><th>{labels.issue_tracker}</th><th /></tr></thead><tbody>
              {drafts.map((draft, index) => <tr key={draft.id}>
                <td>{index + 1}</td>
                <td><input ref={index === 0 ? firstSubjectRef : undefined} data-subtask-id={draft.id} aria-label={`${labels.issue_subject} ${index + 1}`} value={draft.subject} onChange={(event) => setDrafts((current) => current.map((item) => item.id === draft.id ? { ...item, subject: event.target.value } : item))} /></td>
                <td><select aria-label={`${labels.issue_tracker} ${index + 1}`} value={draft.trackerId || ''} onChange={(event) => setDrafts((current) => current.map((item) => item.id === draft.id ? { ...item, trackerId: Number(event.target.value) } : item))}>{trackers.map((tracker) => <option key={tracker.id} value={tracker.id}>{tracker.name}</option>)}</select></td>
                <td><button type="button" className="rk-icon-btn" aria-label={`${labels.delete} ${index + 1}`} onClick={() => setDrafts((current) => current.filter((item) => item.id !== draft.id))}>×</button></td>
              </tr>)}
            </tbody></table>
          </div>
          <button type="button" className="button rk-bulk-subtask-add-row" onClick={addRow}>{labels.bulk_subtask_add_row}</button>
        </>
      )}
