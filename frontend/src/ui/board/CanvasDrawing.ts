import type { BoardData, Column, Issue, Lane } from '../../model/board/types';
import type { CardDisplayMode } from '../useKanbanPreferences';
import type { Rect } from './canvasGeometry';
import { rectIntersects, roundedRect } from './canvasGeometry';
import { assessDrop, canDeleteIssue, canEditIssue, getDropHintVisual } from './canvasInteraction';
import { buildSubtaskKey, parseCellKey } from './keys';
import { getMetrics } from './metrics';
import { cellKey, type BoardState } from './state';
import { flattenSubtasks } from '../../model/board/subtasksTree';
import { truncateText, truncateTextLines } from './canvasText';
import { normalizeTrackerId, resolveClosedState, resolveTrackerName, type TrackerCatalog } from '../../model/issue/issue';
...
import type { RectMap } from './HitTestIndex';
import type { DragState } from './CanvasPointerController';
...
export function createSubjectLineMeasurer(ctx: CanvasRenderingContext2D | null): SubjectLineMeasurer | undefined {
  ...
  layout.laneLayouts.forEach((laneLayout) => {
    ...
    columns.forEach((statusId, colIndex) => {
      ...
      const isTarget = drag?.phase === 'dragging'
        && drag.targetCellKey === key
        && targetAssessment?.action === 'dispatch';
      ctx.fillStyle = hintVisual?.tone === 'advisory'
        ? '#fef3c7'
        : hintVisual?.tone === 'allowed'
          ? '#dcfce7'
          : hintVisual?.tone === 'neutral'
            ? '#f1f5f9'
            : isTarget
              ? '#e0f2fe'
              : colBg;
      ...
      if (hintVisual?.glyph) {
        ...
        ctx.textBaseline = 'top';
        ctx.fillText(hintVisual.glyph, cellRect.x + cellRect.width - 18, cellRect.y + 6);
        ctx.restore();
      }
      if (isTarget) {
        ctx.strokeStyle = hintVisual?.tone === 'advisory' ? '#b45309' : '#0369a1';
        ctx.lineWidth = 3;
        ctx.beginPath();
...
function drawStandardCard(
  ...
  if (rectMap) {
    ...
    const isActiveTimer = String(timerSession?.issueId) === String(issue.id);
    if ((isActionIconsVisible || isActiveTimer) && canLogTime) {
      const hiddenActionSlots = isActiveTimer && !isActionIconsVisible
        ? Number(canEditCard) + Number(canDeleteCard)
        : 0;
...
export function readTheme(container: HTMLDivElement | null): CanvasTheme {
  const fallback = {
    bgMain: '#ffffff',
    surface: '#ffffff',
    border: '#e5e7eb',
