...
// Compatibility shape while query-cache writers migrate to the normalized store.
export type BoardData = BoardViewModel & {
  ok: boolean;
...
export type ToolbarViewModel = {
  meta: Pick<Meta, 'project_id' | 'can_move' | 'can_create' | 'can_delete' | 'complete' | 'server_entity_limit'>;
  columns: Column[];
  lists: Lists;
  labels: Record<string, string>;
};
