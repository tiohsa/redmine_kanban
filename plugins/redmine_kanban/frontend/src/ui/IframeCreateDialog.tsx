  ...
  const handleParentCreated = async (parentId: number) => {
    ...
    if (lines.length > 0) {
      try {
        ...
        onSuccess(`チケット #${parentId} と ${lines.length} 件の子チケットを作成しました`);
      } catch (e) {
        onSuccess(`チケット #${parentId} を作成しましたが、子チケットの作成に失敗しました`);
      }
    } else {
      onSuccess(`チケット #${parentId} を作成しました`);
    }
    onClose();
  };
      ...
      <div className="rk-iframe-dialog-container" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rk-create-footer">
          <div className="rk-subtask-input">
            <label className="rk-label">子チケット一括登録 (1行に1件名)</label>
            <textarea
              rows={3}
              value={subtasks}
          ...
          <div className="rk-modal-actions">
            ...
            <button type="button" className="rk-btn rk-btn-primary" onClick={handleSubmit} disabled={isSubmitting}>
              {isSubmitting ? '作成中...' : (labels.create ?? '作成')}
            </button>
          </div>
        </div>
      </div>
