    ...
    const handleUpdateSuccess = async () => {
        ...
        if (lines.length > 0) {
            try {
                ...
                onSuccess(`チケット #${issueId} を更新し、${lines.length} 件の子チケットを作成しました`);
            } catch (e) {
                onSuccess(`チケット #${issueId} を更新しましたが、子チケットの作成に失敗しました`);
            }
        } else {
            onSuccess(`チケット #${issueId} を更新しました`);
        }
        onClose();
    };
                ...
                <div className="rk-create-footer">
                    ...
                    <div className="rk-modal-actions">
                        ...
                        <button type="button" className="rk-btn rk-btn-primary" onClick={handleSubmit} disabled={isSubmitting}>
                            {isSubmitting ? '更新中...' : (labels.save ?? '保存')}
                        </button>
                    </div>
                </div>
            </div>
