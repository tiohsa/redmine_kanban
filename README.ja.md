...
個人ごとの表示設定と停滞検知により、日々のフローを見えやすくします。
...
- `lists.trackers` には、既存の `id/name` に加えて `workflow_status_ids`、`default_status_id`、`available_project_ids` を返します。選択中のトラッカーが1件で作成先プロジェクトでも利用可能な場合だけ、Native Redmineの作成画面へ引き継ぎます。metadataが不完全な場合は従来どおり広い表示を維持します。
- ドラッグ中の色や記号はsnapshot時点のワークフロー参考情報です。同一セルへのNOOPは送信せず、それ以外は既存のMutation経路へ渡してRedmineを最終Authorityとします。ワークフロー拒否時は自動再試行せず、`WORKFLOW_TRANSITION_NOT_ALLOWED` を使って対象Issueを一度だけ再同期します。
