...
一括作成の冪等性は `Rails.cache` に短時間保持します。UI の二重送信、同一ブラウザセッションの再試行、同一 Redmine プロセス内の重複処理を抑止し、共有 atomic CacheStore では複数プロセス間も調整できます。これは永続的な exactly-once 保証ではないため、別プロセスの MemoryStore、キャッシュ消失、サーバー再起動をまたぐ履歴は保証対象外です。

このプラグインはデータベースマイグレーションや独自テーブルを追加しません。インストール・更新時は Redmine 本体の標準マイグレーションのみ実行してください。
