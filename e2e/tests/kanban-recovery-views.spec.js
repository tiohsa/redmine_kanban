...
test('a narrow saved view recovers a fresh failed snapshot and retains its saved source', async ({ page, baseURL }) => {
  ...
  await menu.getByRole('combobox', { name: l.saved_views_select }).selectOption('narrow');
  const requests = [];
  page.on('request', (r) => { if (/\/kanban\/data\?/.test(r.url())) requests.push(r.url()); });
  const recovered = page.waitForResponse((r) => isSnapshot(r) && r.ok());
  await menu.getByRole('button', { name: l.saved_views_apply }).click();
...
test('toolbar keyboard activation, focus return, outside click and saved view operations', async ({ page, baseURL }) => {
  ...
  const views = page.getByRole('dialog', { name: l.saved_views });
  ...
  await views.getByRole('button', { name: l.saved_views_apply, exact: true }).click();
  await page.keyboard.press('Escape');
  await display.click();
  ...
  await views.getByRole('button', { name: l.saved_views_rename }).click();
  ...
  await expect(views.getByRole('combobox', { name: l.saved_views_select })).toHaveValue('');
